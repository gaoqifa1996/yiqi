...
from readxml import xml2dict, readjson, xml2dict2,dict2xml
import time
import datetime
from map import get_kinident, get_bodyident


############  connect MQ and put/get string ############
def connect():
    recv_mq = {
        "ip": "10.232.129.54",
        "port": "1416",
...
############  prepare or achieve info from sql ##########
...
###########  create xml or interpretation xml ###########
...
def get_emergency(client, cfg):
    ...
    if info is not None:
        ...
        if "start" in data1 and "end" in data1 and "data" in data1:
            ...
            return data1["data"][0]
    return None
def put_emergency_print(client, cfg,recv_mq):
    data = get_emergency(client, cfg)
    if data is None:
        return
    doc = Document()
    env = doc.createElement("env:Envelope")
...
def put_getplan(x, y):
    ...
    x = str(y)
    plan_ack = dict2xml(x)
    connectmq_put(recv_mq, plan_ack)
    print(plan_ack)
